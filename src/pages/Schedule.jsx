...
const SchedulePage = () => {
    ...
    const handleSchedule = (e) => {
        e.preventDefault();
        // Redirect back home for now after slight delay
        setTimeout(() => {
            navigate('/');
        }, 1000);
    };
        ...
        <div className="fade-in-up auth-layout" style={{ minHeight: '100vh', display: 'flex', flexWrap: 'wrap', position: 'relative', overflow: 'hidden', background: '#000' }}>
            ...
            <div className="auth-brand-side">
                <div className="brand-content">
                    ...
                    <h1 className="epic-heading" style={{ fontSize: 'clamp(3rem, 6vw, 5rem)' }}>STRATEGIC<br />ALIGNMENT</h1>
                    <p className="auth-desc">
                        Connect directly with our architects. We don't do sales pitches; we do intelligence audits. Prepare your objectives, state your friction points, and we will formulate an immediate tactical response.
                    </p>
                    ...
                    <div className="schedule-perks">
                        <div className="perk">
                            ...
                            <div>
                                <h4>DIRECT ACCESS</h4>
                                <p>Speak with the lead developers and strategists constructing the Goocranti ecosystem.</p>
                            </div>
                        </div>
                        <div className="perk">
                            <span className="perk-icon">🛡️</span>
                            <div>
                                <h4>SECURE BRIEFING</h4>
                                <p>All operations discussed are strictly confidential under default NDA.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="auth-form-side">
                <div className="auth-form-container premium-glass-card" style={{ maxWidth: '600px', padding: '3.5rem' }}>
                    <h2 className="auth-title">REQUEST BRIEFING</h2>
                    <p className="auth-subtitle">Select your optimal deployment window.</p>
                    ...
                    <form onSubmit={handleSchedule} className="auth-form">
                        <div className="form-row">
                            ...
                            <div className="form-group flex-1">
                                ...
                                <input type="text" placeholder="Stealth Mode LLC" className="elegant-input" />
                            </div>
                        </div>
                        ...
                        <div className="time-selection-grid">
                            <div className="form-group">
                                ...
                                <div className="date-pills">
                                    <div className={`date-pill ${selectedDate === 'Today' ? 'active' : ''}`} onClick={() => setSelectedDate('Today')}>Today</div>
                                    <div className={`date-pill ${selectedDate === 'Tomorrow' ? 'active' : ''}`} onClick={() => setSelectedDate('Tomorrow')}>Tomorrow</div>
                                    <div className={`date-pill ${selectedDate === 'Next Week' ? 'active' : ''}`} onClick={() => setSelectedDate('Next Week')}>Next Week</div>
                                </div>
                            </div>
                        </div>

                        <button type="submit" className="btn-iconic" style={{ width: '100%', marginTop: '1.5rem' }}>CONFIRM DEPLOYMENT →</button>
                    </form>
                </div>
            </div>
            ...
            <style>{`
                ...
                .epic-heading {
                    font-family: var(--font-impact); color: #fff; margin-bottom: 2rem; line-height: 1; letter-spacing: 4px;
                }

                .auth-desc {
                    font-size: 1.15rem; color: rgba(255,255,255,0.7); line-height: 1.8; font-weight: 300; margin-bottom: 3rem;
                }
                
                .schedule-perks { display: flex; flex-direction: column; gap: 2rem; }
                .perk { display: flex; gap: 1.5rem; align-items: flex-start; }
                .perk-icon { font-size: 1.8rem; line-height: 1; }
                .perk h4 { color: #fff; font-family: var(--font-heading); margin: 0 0 0.5rem 0; letter-spacing: 2px; }
                .perk p { color: rgba(255,255,255,0.5); font-weight: 300; margin: 0; line-height: 1.5; font-size: 0.9rem; }
                ...
                .elegant-input {
                    ...
                    padding: 1.2rem 1.5rem; border-radius: 12px; color: #fff; font-size: 1rem;
                    outline: none; transition: all 0.3s; font-family: var(--font-body); width: 100%;
                }

                .elegant-input:focus { border-color: rgba(20, 184, 166, 0.5); background: rgba(0,0,0,0.6); box-shadow: 0 0 15px rgba(20, 184, 166, 0.15);}
                
                .date-pills { display: flex; gap: 1rem; }
                .date-pill {
                    flex: 1; padding: 1rem; text-align: center; background: rgba(0,0,0,0.4); border: 1px solid rgba(255,255,255,0.1);
                    border-radius: 12px; color: rgba(255,255,255,0.7); cursor: pointer; transition: all 0.3s;
                }
                .date-pill:hover { border-color: rgba(255,255,255,0.3); color: #fff; }
                .date-pill.active { background: rgba(20, 184, 166, 0.2); border-color: var(--primary-teal); color: #fff; box-shadow: 0 0 15px rgba(20, 184, 166, 0.15); }

                .btn-iconic {
                    background: #fff; color: #000; border: none; padding: 1.2rem 3rem; border-radius: 50px;
                    font-family: var(--font-heading); font-size: 1.1rem; font-weight: 700; letter-spacing: 2px;
